...
import { setRestaurant } from "../redux/feature/restaurantSlice";

const RestaurantScreen = ({ route }) => {
  const { id, imageUrl, title, rating } = route.params;
  const navigation = useNavigation();
  const basketItems = useSelector((state) => state.basket.item);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(setRestaurant({ id, imageUrl, title, rating }));
  }, [dispatch]);
    ...
    <>
      ...
      <ScrollView>
        ...
        <View className="pb-32">
          ...
          <DishRow id={1} imageUrl={imageUrl} title={"Yo! Sushi"} />
          <DishRow id={2} imageUrl={imageUrl} title={"Chickens"} />
          <DishRow id={3} imageUrl={imageUrl} title={"Family Platter"} />
        </View>
      </ScrollView>
    </>
  );
